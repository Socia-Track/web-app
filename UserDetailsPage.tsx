    ...
    return (
      <div className="flex h-screen items-center justify-center bg-background">
        <div className="text-center">
          ...
          <h2 className="text-lg font-semibold text-white">Loading User Details...</h2>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-background">
        <div className="text-center max-w-md">
          <h2 className="text-xl font-bold text-white mb-2">Error Loading User Details</h2>
        ...
        >
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground mt-1">{stats.activeCampaigns} active</p>
            </CardContent>
          </Card>

          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-xs font-medium text-muted-foreground">Links</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-foreground">{stats.totalClicks}</div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-xs font-medium text-muted-foreground">Attributions</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-foreground">{stats.totalAttributions}</div>
            </CardContent>
          </Card>
                          ...
                          {links.map((link) => (
                            <tr key={link.id} className="border-b border-border">
                              <td className="p-2 text-sm">
                                ...
                                {link.linkName && (
                                  <div className="text-xs text-muted-foreground">{link.linkName}</div>
                                )}
                              </td>
                              <td className="p-2">
                    ...
                    <div className="space-y-3">
                      {transactions.map((tx) => (
                        <div key={tx.id} className="border border-border rounded-lg p-4">
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <div className="font-mono text-sm text-foreground">{tx.transactionHash.slice(0, 20)}...</div>
                              <div className="text-xs text-muted-foreground mt-1">Wallet: {tx.walletAddress.slice(0, 10)}...{tx.walletAddress.slice(-8)}</div>
                              {tx.campaignName && (
                                <div className="text-xs text-muted-foreground mt-1">Campaign: {tx.campaignName}</div>
                              )}
                            </div>
                            <div className="text-right">
                              <div className="font-bold text-foreground">{tx.amount.toFixed(4)} {tx.currency}</div>
              ...
              <Card>
                ...
                <CardContent>
                  {attributions.length === 0 ? (
                    <p className="text-center text-muted-foreground py-8">No attributions found</p>
                  ) : (
                    <div className="space-y-3">
                      {attributions.map((attr) => (
                        <div key={attr.id} className="border border-border rounded-lg p-3">
                          <div className="flex items-center justify-between">
                            <div>
                              ...
                              {attr.campaignName && (
                                <div className="text-sm text-muted-foreground mt-1">Campaign: {attr.campaignName}</div>
                              )}
                              {attr.influencer && (
                                <div className="text-sm text-muted-foreground">Influencer: {attr.influencer}</div>
                              )}
                            </div>
